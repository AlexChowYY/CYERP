...
public class LatestFragment extends BaseFragment {
    ...
    @BindView(R.id.rv_fragment_latest)
    RecyclerView mRV;
    @BindView(R.id.trk)
        ...
        if (mAdapter == null) {
            mAdapter = new RecyclerCommonAdapter<FaceBean>(mActivity, R.layout.item_face, mFaceList) {
                ...
                protected void convert(ViewHolder holder, final FaceBean faceBean, final int position) {
                    ...
                    if (Integer.parseInt(faceBean.getNewCount()) > 0) {
                        if (Integer.parseInt(faceBean.getNewCount()) < 10) {
                            newNum.setBackgroundResource(R.drawable.shape_circle_red_15);
                            newNum.setText(faceBean.getNewCount());
                        } else {
                            newNum.setBackgroundResource(R.drawable.shape_ellipse_red_15);
                            if (Integer.parseInt(faceBean.getNewCount())> 99) {
                                newNum.setText("99+");
                            } else {
                                newNum.setText(faceBean.getNewCount());
                            }
                    ...
                    attentionTV.setOnClickListener( new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            getAttention(faceBean.getId(), faceBean.getIsAttention(), attentionTV, position, Integer.parseInt(faceBean.getLove()));
                        }
                    });
                }
            };
                    ...
                    public void onSuccess(String response) {
                        ...
                        try {
                            ...
                            if (data.getInt("resultCode") == 200) {
                                ...
                                EventBus.getDefault().post(new UpdateGroupMemberEvent());
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
